...
describe('Swaps Util', () => {
  ...
  describe('isContractAddressValid', () => {
    ...
    it('returns true if "to" is Rinkeby contract address on Rinkeby network', () => {
      usedTradeTxParams.to =
        SWAPS_CHAINID_CONTRACT_ADDRESS_MAP[CHAIN_IDS.RINKEBY];
      expect(
        isContractAddressValid(usedTradeTxParams.to, CHAIN_IDS.RINKEBY),
      ).toBe(true);
    });
  ...
  describe('getNetworkNameByChainId', () => {
    ...
    it('returns "rinkeby" for Rinkeby chain ID', () => {
      expect(getNetworkNameByChainId(CHAIN_IDS.RINKEBY)).toBe(RINKEBY);
    });
